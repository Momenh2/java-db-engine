...
public class bplustree implements Serializable{
	...
	public void insert(String key1, String value){
		//int prime = 31; // A prime number for hashing
	    int key = 1;
	...
	public String search(String key1) {
	...
	private void printNode(Node node, int level) {
	    if (node instanceof InternalNode) {
	        ...
	        printIndent(level);
	        System.out.println("[");
	        for (int i = internalNode.degree - 1; i >= 0; i--) {
	            printNode(internalNode.childPointers[i], level + 1);
	            printIndent(level + 1);
	            System.out.println("(" + internalNode.keys[i] + ")");
	        }
	        printNode(internalNode.childPointers[internalNode.degree], level + 1);
	        printIndent(level);
	        System.out.println("]");
	        ...
	        for (int i = 0; i < leafNode.numPairs; i++) {
	            System.out.print("(" + leafNode.dictionary[i].key + ", " + leafNode.dictionary[i].value + ")");
	            if (i < leafNode.numPairs - 1) {
	                System.out.print(", ");
	            }
	        }
	        System.out.println("]");
	...
	private void printIndent(int level) {
	    for (int i = 0; i < level; i++) {
	        System.out.print("    "); // Adjust indentation level as needed
	    }
	}
	...
	private void printTree(Node node) {
	    if (node instanceof LeafNode) {
	        LeafNode leafNode = (LeafNode) node;
	        for (int i = 0; i < leafNode.numPairs; i++) {
	            System.out.println(leafNode.dictionary[i].key + ", " + leafNode.dictionary[i].value);
	        }
	    } else if (node instanceof InternalNode) {
	        InternalNode internalNode = (InternalNode) node;
	        for (int i = 0; i < internalNode.degree; i++) {
	            printTree(internalNode.childPointers[i]);
	        }
	    }
	}
	...
	public static void main(String[] args) {
	  ...
	  /*  
	    ...
	    if (bpt.search("ahmed") != null) {
	      System.out.println("Found");
	    } else {
	      System.out.println("Not Found");
	    }
	    if (bpt.search(44) != null) {
		      System.out.println("Found");
		    } else {
		      System.out.println("Not Found");
		    }
	    if (bpt.search("232") != null) {
		      System.out.println("Found");
		    } else {
		      System.out.println("Not Found");
		    }
